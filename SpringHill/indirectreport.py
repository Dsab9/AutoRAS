...
#def hecras_report(output_dir):
output_dir = Path(r'D:\HEC-RAS Projects\WF_Rock_Cr\20220613_Indirect')

p1hdf_filename = Path(output_dir / 'WestForkRockCree.p02.hdf')
p2hdf_filename = Path(output_dir / 'WestForkRockCree.p01.hdf')

mod_results = h5py.File(p1hdf_filename, 'r')
...
up_bc = mod_results['Results/Unsteady/Output/Output Blocks/Base Output/Unsteady Time Series/Boundary Conditions/Upstream Inflow']
dwn_bc = mod_results['Event Conditions/Unsteady/Boundary Conditions/Normal Depths/2D: 2D Flow Area BCLine: Downstream Outflow']
...
mruns_filename = Path(output_dir / 'model_iterations.csv')
cal_params_filename = Path(output_dir / 'simplex_dataframe.csv')""".format(str(output_dir))
...
cal_params = pd.read_csv(cal_params_filename)
cal_params = cal_params.columns[:-1].tolist()"""

code4 = """\
mruns_cal_params = mruns[cal_params + ['iteration', 'Obj_Func_Value']]
mruns_cp_tidy = pd.melt(mruns_cal_params,
                        ['iteration', 'Obj_Func_Value', 'Q'],
...
code14 = """\
fig = ff.create_distplot([mruns['Q'].loc[14:].tolist()], ['Q Distribution'])
...
code15 = """\
print("The median Q is: {0} m^3/s --- {1} cfs".format(mruns['Q'].loc[14:].median(), mruns['Q'].loc[14:].median() / 0.3048**3))
print("The mean Q is: {0} m^3/s --- {1} cfs".format(mruns['Q'].loc[14:].mean(), mruns['Q'].loc[14:].mean() / 0.3048**3))
print("The standard deviation of Q is: {0} m^3/s --- {1} cfs".format(mruns['Q'].loc[14:].std(), mruns['Q'].loc[14:].std() / 0.3048**3))"""
...
nb_fname = Path(r'C:\Users\CNB968\OneDrive - MT\Documents\Jupyter Notebooks\HECRAS_20220613_Indirect_Report.ipynb')
