...
def run_ras(prj_path):
    ...
    RASProject = os.path.join(os.getcwd(), prj_path)
...
def get_2D_refinement_region(geometry_hdf):
...
def load_current_plan_results(currentPlanFile):
    """
...
def extract_WSE_timeseries(grid_cells, currentPlanFile):
    wat_el = load_current_plan_results(currentPlanFile)
    ref_pnts = wat_el[grid_cells, :]

    return ref_pnts


def get_Mannings_calibration_regions(geometry_hdf):
    """
...
def change_Base_Mannings(geometry_hdf, new_param_dict):
    """
...
def assign_param_vals(n, params_list):
